...
### 2C. `pg_restore` de dump manual — DR total / fora do Supabase
```bash
...
SELECT count(*) FROM information_schema.tables
  WHERE table_schema='public' AND table_type='BASE TABLE';   -- esperado: 18
SELECT count(*) FROM produtos;   -- esperado: ~37
SELECT count(*) FROM pedidos;    -- >= valor do dia do backup
SELECT migration_name FROM _prisma_migrations ORDER BY finished_at;  -- 4 migrations
```
...
- [ ] `18` tabelas no schema `public`; `4` migrations em `_prisma_migrations`.
